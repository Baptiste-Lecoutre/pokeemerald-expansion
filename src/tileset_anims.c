...
static void TilesetAnim_Verdanturf(u16);
...
void InitTilesetAnim_Verdanturf()
{
    sSecondaryTilesetAnimCounter = sPrimaryTilesetAnimCounter;
    sSecondaryTilesetAnimCounterMax = sPrimaryTilesetAnimCounterMax;
    sSecondaryTilesetAnimCallback = TilesetAnim_Verdanturf;
}
...
static void TilesetAnim_Verdanturf(u16 timer)
{
    if (timer % 8 == 0)
        QueueAnimTiles_Mauville_Flowers(timer / 8, 0);
    if (timer % 8 == 1)
...
static void QueueAnimTiles_General_LandWaterEdge(u16 timer)
{
    ...
    AppendTilesetAnimToBuffer(gTilesetAnims_General_LandWaterEdge[i], (u16 *)(BG_VRAM + TILE_OFFSET_4BPP(480)), 16 * TILE_SIZE_4BPP);
}

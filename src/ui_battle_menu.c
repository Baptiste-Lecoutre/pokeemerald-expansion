...
static const u32 sMenuTiles[]      = INCBIN_U32("graphics/battle_menu/tiles.4bpp.lz");
static const u8 sStatDownArrow[]   = INCBIN_U8("graphics/battle_menu/stat_down_arrow.4bpp");
static const u8 sStatUpArrow[]     = INCBIN_U8("graphics/battle_menu/stat_up_arrow.4bpp");
static const u8 sCheck[]           = INCBIN_U8("graphics/battle_menu/check.4bpp");

//Palettes
static const u16 sMenuPalette[]        = INCBIN_U16("graphics/battle_menu/palette.gbapal");
static const u16 sMenuPalette_Blue[]   = INCBIN_U16("graphics/battle_menu/palette_blue.gbapal");
static const u16 sMenuPalette_Yellow[] = INCBIN_U16("graphics/battle_menu/palette_yellow.gbapal");
static const u16 sMenuPalette_Green[]  = INCBIN_U16("graphics/battle_menu/palette_green.gbapal");
static const u16 sMenuPalette_Red[]    = INCBIN_U16("graphics/battle_menu/palette_red.gbapal");

//Battler Tabs
static const u32 sMenu_Tilemap_Singles_Battler_Status[]     = INCBIN_U32("graphics/battle_menu/titlemap_singles_battler_status.bin.lz");
static const u32 sMenu_Tilemap_Doubles_Battler_Status[]     = INCBIN_U32("graphics/battle_menu/titlemap_doubles_battler_status.bin.lz");
//
static const u32 sMenu_Tilemap_Singles_Battler_Abilities[]  = INCBIN_U32("graphics/battle_menu/titlemap_singles_battler_abilities.bin.lz");
static const u32 sMenu_Tilemap_Doubles_Battler_Abilities[]  = INCBIN_U32("graphics/battle_menu/titlemap_doubles_battler_abilities.bin.lz");
//Field Tabs
static const u32 sMenu_Tilemap_Singles_Field[]              = INCBIN_U32("graphics/battle_menu/titlemap_singles_field.bin.lz");
static const u32 sMenu_Tilemap_Doubles_Field[]              = INCBIN_U32("graphics/battle_menu/titlemap_doubles_field.bin.lz");
//
static const u32 sMenu_Tilemap_Singles_Speed[]              = INCBIN_U32("graphics/battle_menu/titlemap_singles_field_speed.bin.lz");
static const u32 sMenu_Tilemap_Doubles_Speed[]              = INCBIN_U32("graphics/battle_menu/titlemap_doubles_field_speed.bin.lz");
...
#define BATTLER_HAS_ABILITY(battlerId, ability) (GetBattlerAbility(battlerId) == ability && IsBattlerAlive(battlerId))
...
static void CalculateDamage(u8 battler, u8 target, u8 moveIndex){
    ...
    targetParty = GetBattlerParty(target);

    natureAtk = GetMonData(&party[gBattlerPartyIndexes[battler]], MON_DATA_HIDDEN_NATURE, NULL);
...
static void PrintDamageCalculation(u8 battler, u8 target, u8 moveIdx){
    ...
    //Third Part
    StringCopy(gStringVar1, gStringVar4);
    StringCopy(gStringVar2, gSpeciesInfo[gBattleMons[target].species].speciesName);
...
const u8 sText_Title_Field_B_WEATHER_STRONG_WINDS[]         = _("Strong Winds");
const u8 sText_Title_Field_B_WEATHER_RAIN_PRIMAL[]          = _("Primal Rain");
const u8 sText_Title_Field_B_WEATHER_SUN_PRIMAL[]           = _("Primal Sun");
...
static void PrintFieldTab(void)
{
        ...
        switch(sMenuDataPtr->fieldInfo[(i + sMenuDataPtr->currentFieldInfo) % sMenuDataPtr->numFields]){
            case FIELD_INFO_WEATHER:
                ...
                if((gBattleWeather & B_WEATHER_RAIN) && !(gBattleWeather & B_WEATHER_RAIN_PRIMAL))
                    ...
                    StringCopy(gStringVar1, sText_Title_Field_B_WEATHER_STRONG_WINDS);
                else if((gBattleWeather & B_WEATHER_RAIN_PRIMAL))
                    StringCopy(gStringVar1, sText_Title_Field_B_WEATHER_RAIN_PRIMAL);
                else if((gBattleWeather & B_WEATHER_SUN_PRIMAL))
                    StringCopy(gStringVar1, sText_Title_Field_B_WEATHER_SUN_PRIMAL);
                else
                ...
                if((gBattleWeather & B_WEATHER_ANY)){
                    //Turns Left
                    if(!(gBattleWeather & B_WEATHER_RAIN_PRIMAL) && !(gBattleWeather & B_WEATHER_SUN_PRIMAL)){
    ...
    StringCopy(gStringVar1, gMoveNames[MOVE_THUNDERBOLT]);
...
static void PrintSpeedTab(void)
{
    ...
    for (i = 0; i < gBattlersCount - 1; i++)
    {
        ...
        {
            if (GetWhichBattlerFaster(sBattlerByTurnOrder[i], sBattlerByTurnOrder[j], TRUE) != 0)
            {

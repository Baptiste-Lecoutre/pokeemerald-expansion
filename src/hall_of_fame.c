...
static void Task_Hof_InitMonData(u8 taskId)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES))
        {
            ...
            formChanges = GetSpeciesFormChanges(sHofMonPtr->mon[i].species);
            if (formChanges != NULL)
            {
...
static void Task_Hof_DisplayPlayer(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].tPlayerSpriteID = CreateTrainerPicSprite(GetTrainerFrontSpriteBasedOnPlayerCostumeAndGender(gSaveBlock2Ptr->costumeId, gSaveBlock2Ptr->playerGender), TRUE, 120, 72, 6, TAG_NONE);

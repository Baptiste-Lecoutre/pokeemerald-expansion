...
#define TAG_FRIENDSHIP_ICON 30013
#define TAG_TERA_TYPE 30014
#define TAG_MON_SHADOW 30015
...
static const union AnimCmd sSpriteAnim_FriendshipIcon0old[] =
{
    ANIMCMD_FRAME(0, 0),
    ANIMCMD_END
};

static const union AnimCmd sSpriteAnim_FriendshipIcon1old[] =
{
    ANIMCMD_FRAME(1, 0),
    ANIMCMD_END
};

static const union AnimCmd sSpriteAnim_FriendshipIcon2old[] =
{
    ANIMCMD_FRAME(2, 0),
    ANIMCMD_END
};

static const union AnimCmd sSpriteAnim_FriendshipIcon3old[] =
{
    ANIMCMD_FRAME(3, 0),
    ANIMCMD_END
};

static const union AnimCmd sSpriteAnim_FriendshipIcon4old[] =
{
    ANIMCMD_FRAME(4, 0),
    ANIMCMD_END
};
...
{
    sSpriteAnim_FriendshipIcon0old,
    sSpriteAnim_FriendshipIcon1old,
    sSpriteAnim_FriendshipIcon2old,
    sSpriteAnim_FriendshipIcon3old,
    sSpriteAnim_FriendshipIcon4old,
};
...
static void RunMonAnimTimer(void)
{
    ...
    {
        gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON]].data[0] = sMonSummaryScreen->summary.species2;
        if (BW_SUMMARY_MON_SHADOWS)
            gSprites[sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_SHADOW]].data[0] = sMonSummaryScreen->summary.species2;
...
static void CreateMoveTypeIcons(void)
{
    ...
    for (i = SPRITE_ARR_ID_TERA_TYPE; i < SPRITE_ARR_ID_TYPE + TYPE_ICON_SPRITE_COUNT; i++)
    {
        if (sMonSummaryScreen->spriteIds[i] == SPRITE_NONE)
        {
            ...
            {
                if (P_SHOW_TERA_TYPE >= GEN_9)
                    sMonSummaryScreen->spriteIds[i] = CreateSprite(&sSpriteTemplate_MoveTypesSmall, 0, 0, 2); //CreateSprite(&sSpriteTemplate_TeraType, 0, 0, 2);
            }
            else
            {
...
static void SetMonTypeIcons(void)
{
    ...
    else
    {
        ...
        {
            SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].types[1], 104, 46, SPRITE_ARR_ID_TYPE + 1); // 106 // 108
        ...
        {
            SetTypeSpritePosAndPal(summary->teraType, (gSpeciesInfo[summary->species].types[0] != gSpeciesInfo[summary->species].types[1]) ? 131 : 95, 46, SPRITE_ARR_ID_TERA_TYPE); //140 //200 //36, 47
        }
    }
}
...
static void SetFriendshipSprite(void)
{
    ...
    if (sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_FRIENDSHIP] == SPRITE_NONE)
        sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_FRIENDSHIP] = CreateSprite(&sSpriteTemplate_FriendshipIcon, 234, 50, 0); //153, 25
...
static void CreateCaughtBallSprite(struct Pokemon *mon)
{
    ...
    if (friendship > 30)
    {
		...
		sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_HEART] = CreateSprite(&sSpriteTemplate_FriendshipIcon, 234, 50, 0);
		if (sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_HEART] < MAX_SPRITES)
		{
			u16 imageNum = 0;

...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (gSaveBlock2Ptr->optionsShowFollowerPokemon && (GetMonData(&mons[slotId], MON_DATA_SPECIES) != SPECIES_NONE || GetMonData(&mons[slotId], MON_DATA_IS_EGG) == FALSE))
...
//    AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_STAT_EDIT); // works as intended
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, TRUE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CONFIG_START_MENU_FULL ? CB2_ReturnToFullScreenStartMenu : CB2_ReturnToFieldWithOpenMenu);
}
...
static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, struct PartyMenuBox *menuBox, u8 priority)
{
    if (species != SPECIES_NONE)
    {
        menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0] + 4/*2*/, menuBox->spriteCoords[1] - 1, 4, pid);
...
static void CreatePartyMonHeldItemSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox)
{
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        menuBox->megaStoneSpriteId = CreateSprite(&sSpriteTemplate_MegaStone, menuBox->spriteCoords[2] + 8, menuBox->spriteCoords[3] - 2, 0);
        menuBox->zCrystalSpriteId = CreateSprite(&sSpriteTemplate_ZCrystal, menuBox->spriteCoords[2] + 8, menuBox->spriteCoords[3], 0);
...
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CONFIG_START_MENU_FULL ? CB2_ReturnToFullScreenStartMenu : CB2_ReturnToFieldWithOpenMenu);
}
...
{
    StatEditor_Init(ChangePokemonStatsPartyScreen_CB, gSpecialVar_0x8004);
}

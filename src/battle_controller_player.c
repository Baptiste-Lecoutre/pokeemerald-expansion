...
static void CompleteShowEnemyParty(u32 battler) 
{
    ...
    else if (JOY_NEW(B_BUTTON) || gPlayerDpadHoldFrames > 59)
    {
        ...
        else if (B_QUICK_MOVE_CURSOR_TO_RUN)
        {
            if (!(gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_RAID))) // If wild battle, pressing B moves cursor to "Run".
            {
                PlaySE(SE_SELECT);
                ActionSelectionDestroyCursorAt(gActionSelectionCursor[battler]);
                gActionSelectionCursor[battler] = B_ACTION_RUN;
                ActionSelectionCreateCursorAt(gActionSelectionCursor[battler], 0);
            }
        }
    }
...
static void MoveSelectionDisplayMoveDescription(u32 battler)
{
    ...
    {*/
        pwr = GetMovePower(move); // for base power without modifiers
        acc = GetMoveAccuracy(move); // for base accuracy without modifiers
    //}

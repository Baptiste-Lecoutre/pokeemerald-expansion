...
static void StatEditor_DrawMonIcon(u16 dexNum);
...
static bool8 StatEditor_DoGfxSetup(void)
{
    ...
    case 4:
        ...
        StatEditor_DrawMonIcon(sStatEditorDataPtr->speciesID);
        gMain.state++;
        break;
    case 5:
...
static bool8 StatEditor_LoadGraphics(void)
{
    ...
    case 1:
        ...
        {
            LZDecompressWram(sStatEditorBgTilemap, sStatEditorDataPtr->sBg1TilemapBuffer);
            sStatEditorDataPtr->gfxLoadState++;
        }
        break;
    case 2:
    ...
    case 4:
        if (FreeTempTileDataBuffersIfPossible() != TRUE)
        {
            LZDecompressWram(gScrollBgTilemap, sStatEditorDataPtr->sBg3TilemapBuffer);
            sStatEditorDataPtr->gfxLoadState++;
        }
        break;
    default:
...
static void StatEditor_DrawMonIcon(u16 dexNum)
{
    ...
    bool32 isShiny = GetMonData(ReturnPartyMon(), MON_DATA_IS_SHINY);
    u32 personality = GetMonData(ReturnPartyMon(), MON_DATA_PERSONALITY);

    POKEMON_SPRITE = CreateMonPicSprite_Affine(speciesId, isShiny, personality, MON_PIC_AFFINE_FRONT, MON_ICON_X, MON_ICON_Y, 0, TAG_NONE);
    ...
    SHADOW_SPRITE = CreateMonPicSprite_Affine(speciesId, isShiny, personality, MON_PIC_AFFINE_FRONT, MON_ICON_X + 5, MON_ICON_Y - 2, 0, TAG_NONE);

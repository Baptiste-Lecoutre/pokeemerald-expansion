...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_SUPERSWEET_SYRUP:
            if (!gSpecialStatuses[battler].switchInAbilityDone
                    && !(gBattleStruct->supersweetSyrup[battler] & gBitTable[gBattlerPartyIndexes[battler]]))
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattleStruct->supersweetSyrup[battler] |= gBitTable[gBattlerPartyIndexes[battler]];
        ...
        case ABILITY_INTREPID_SWORD:
            if (!gSpecialStatuses[battler].switchInAbilityDone && CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN)
                 && !(gBattleStruct->intrepidSwordBoost[battler] & gBitTable[gBattlerPartyIndexes[battler]]))
            {
        ...
        case ABILITY_DAUNTLESS_SHIELD:
            if (!gSpecialStatuses[battler].switchInAbilityDone && CompareStat(battler, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN)
                 && !(gBattleStruct->dauntlessShieldBoost[battler] & gBitTable[gBattlerPartyIndexes[battler]]))
            {
                gBattleScripting.savedBattler = gBattlerAttacker;
                gBattlerAttacker = battler;
        ...
        case ABILITY_ZERO_TO_HERO:
            ...
            if (!gSpecialStatuses[battler].switchInAbilityDone
             ...
             && !(gBattleStruct->transformZeroToHero[battler] & gBitTable[gBattlerPartyIndexes[battler]]))
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                gBattleStruct->transformZeroToHero[battler] |= gBitTable[gBattlerPartyIndexes[battler]];
        ...
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_CUD_CHEW:
                if (gDisableStructs[battler].cudChew == TRUE)
                {
                    ...
                    gLastUsedItem = gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerPosition(battler)];
                    gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler]][GetBattlerPosition(battler)] = ITEM_NONE;
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_RAGE_FIST:
        basePower += 50 * gBattleStruct->timesGotHit[battlerAtk][gBattlerPartyIndexes[battlerAtk]];
...
{
    if ((B_FLAG_INVERSE_BATTLE != 0 && FlagGet(B_FLAG_INVERSE_BATTLE)) != (gSaveBlock2Ptr->optionsTypeChart == 1))

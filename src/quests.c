...
static const u16 sPopUpWindowBorder_Palette[] = INCBIN_U16("graphics/map_popup/bw/black.gbapal");
...
static u8 *GetQuestPointer(u16 id)
{
    if (id > NUM_QUESTS - 1)
        return NULL;
    return &gSaveBlock3Ptr->quests[id / 8];
}
...
static void ShowQuestPopUpWindow(void)
{
    ...
    LoadPalette(sPopUpWindowBorder_Palette, BG_PLTT_ID(14), PLTT_SIZE_4BPP);
...
static void LoadQuestPopUpWindowBgs(void)
{
    u8 primaryWindowId = GetMapNamePopUpWindowId();

...
void BufferStringBattle(u16 stringID, u32 battler)
{
    ...
    case STRINGID_INTROMSG: // first battle msg
        ...
        else
        {
            ...
            else if (IsDoubleBattle() && IsValidForBattle(&gEnemy2Party[gBattlerPartyIndexes[GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT)]]))
    ...
    case STRINGID_INTROSENDOUT: // poke first send-out
        ...
        {
            if (IsDoubleBattle() && IsValidForBattle(&partnerParty[gBattlerPartyIndexes[BATTLE_PARTNER(battler)]]) && !FlagGet(FLAG_SOOTOPOLIS_BATTLE))
            {
        ...
        {
            if (IsDoubleBattle() && IsValidForBattle(&partnerParty[gBattlerPartyIndexes[BATTLE_PARTNER(battler)]]))
            {
                if (BATTLE_TWO_VS_ONE_OPPONENT)

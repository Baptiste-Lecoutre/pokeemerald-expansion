...
static void Cmd_adjustdamage(void)
{
    ...
    AdjustRaidBossDamage(gBattlerAttacker, gBattlerTarget);
}
...
static void UpdateSentMonFlags(u32 battler)
{
    ...
    if (GetBattlerSide(battler) == B_SIDE_OPPONENT && IsBattlerAlive(battler))
        gBattleStruct->revealedEnemyMons |= 1u << gBattlerPartyIndexes[battler]; //Add to team preview
}
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        if (odds > 254) // mon caught
        {
            ...
            else if (ballId == BALL_DREAM) // Give Pokemon their Hidden Ability when caught in a Dream Ball
            {   
                u32 Ability = 2;
...
static void Cmd_givecaughtmon(void)
{
    ...
    case GIVECAUGHTMON_GIVE_AND_SHOW_MSG:
        if (B_RESTORE_HELD_BATTLE_ITEMS >= GEN_9)
        {
            ...
            if (lostItem != ITEM_NONE && ItemId_GetPocket(lostItem) != POCKET_BERRIES) // also restores raid boss item

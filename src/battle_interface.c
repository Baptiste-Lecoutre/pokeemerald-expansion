...
u8 CreateBattlerHealthboxSprites(u8 battlerId)
{
    ...
    {
        gBattleStruct->raid.boss[battlerId].bossState |= RAID_RESHOW_SHIELD;
        UpdateRaidShield();
    }
    if (IsRaidBoss(battlerId) && (gBattleStruct->raid.boss[battlerId].bossState & RAID_INTRO_COMPLETE)) // à modifier
...
void SetHealthboxSpriteInvisible(u8 healthboxSpriteId)
{
    ...
    if (IsRaidBoss(battlerId))
    {
        ...
        if ((gBattleStruct->raid.boss[battlerId].bossState & RAID_INTRO_COMPLETE))
...
static void TryToggleHealboxVisibility(u32 priority, u32 healthboxLeftSpriteId, u32 healthboxRightSpriteId, u32 healthbarSpriteId)
{
    ...
    if (gBattleStruct->raid.raidState & RAID_CATCHING_BOSS && !invisible) // catching pesky bug
        return;
...
{
    u32 s;
    u32 speedScale = SpeedUp_GetBattleSpeedScale(TRUE);
...
static void MoveBattleBarGraphically(u8 battlerId, u8 whichBar)
{
    ...
    case HEALTH_BAR:
        ...
        if (IsRaidBoss(battlerId) && gRaidTypes[gRaidData.raidType].shieldType == RAID_SHIELD_MAX)
            barElementId = HEALTHBOX_GFX_HP_BAR_RED;
        else if (IsRaidBoss(battlerId) && gRaidTypes[gRaidData.raidType].shieldType == RAID_SHIELD_TERA)
            barElementId = HEALTHBOX_GFX_HP_BAR_YELLOW; //HEALTHBOX_GFX_12; // Exp bar for blue color
        else if (IsRaidBoss(battlerId) && gRaidTypes[gRaidData.raidType].shieldType == RAID_SHIELD_MEGA)
...
static const struct SpriteTemplate sSpriteTemplate_LastUsedBallWindow =
{
    ...
    .paletteTag = LAST_BALL_WINDOW_TAG,
...
#endif
static const u16 sLastUsedBallWindowPalette[] = INCBIN_U16("graphics/battle_interface/last_used_ball.gbapal");
...
{
    sLastUsedBallWindowPalette, LAST_BALL_WINDOW_TAG
};
...
static void DestroyLastUsedBallWinGfx(struct Sprite *sprite)
{
    ...
    FreeSpritePaletteByTag(LAST_BALL_WINDOW_TAG);
    DestroySprite(sprite);
    gBattleStruct->ballSpriteIds[1] = MAX_SPRITES;
}
...
{
    [TYPE_NONE] =       type_icon_frame(gTypeIconsTiles, TYPE_NONE),
...
static const struct SpriteTemplate sSpriteTemplate_MoveInfoWindowCustom =
{
...
static const u8 sMoveInfoWindowGfxCustom[] = INCBIN_U8("graphics/battle_interface/move_desc_trigger_l.4bpp");

static const struct SpriteSheet sSpriteSheet_MoveInfoWindowCustom =
{
    sMoveInfoWindowGfxCustom, sizeof(sMoveInfoWindowGfxCustom), MOVE_INFO_WIN_TAG
};
...
void TryLoadMoveInfoWindow(u32 battler)
{
    ...
    if (GetSpriteTileStartByTag(MOVE_INFO_WIN_TAG) == 0xFFFF)
        LoadSpriteSheet(&sSpriteSheet_MoveInfoWindowCustom);

    spriteId = CreateSpriteAtEnd(&sSpriteTemplate_MoveInfoWindowCustom, MOVE_INFO_WIN_X_0, MOVE_INFO_WIN_Y, 0xFF);

...
static void Task_GiveExpWithExpBar(u8 taskId)
{
    ...
    else
    {
        ...
        if (r4 == -1)
        {
            ...
            level = GetMonData(&party[monId], MON_DATA_LEVEL);
            currExp = GetMonData(&party[monId], MON_DATA_EXP);
            species = GetMonData(&party[monId], MON_DATA_SPECIES);
            ...
            if (currExp + gainedExp >= expOnNextLvl)
            {
                ...
                SetMonData(&party[monId], MON_DATA_EXP, &expOnNextLvl);
            ...
            {
                currExp += gainedExp;
                SetMonData(&party[monId], MON_DATA_EXP, &currExp);
                gBattlerControllerFuncs[battlerId] = CompleteOnInactiveTextPrinter;
                DestroyTask(taskId);
            }
        }
...
static void Task_UpdateLvlInHealthbox(u8 taskId)
{
    ...
    if (!gBattleSpritesDataPtr->healthBoxesData[battlerId].specialAnimActive)
    {
        ...
        if (IsDoubleBattle() == TRUE && monIndex == gBattlerPartyIndexes[BATTLE_PARTNER(battlerId)])
            UpdateHealthboxAttribute(gHealthboxSpriteIds[BATTLE_PARTNER(battlerId)], &party[monIndex], HEALTHBOX_ALL);
        else
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battlerId], &party[monIndex], HEALTHBOX_ALL);
...
static void PlayerPartnerHandleLoadMonSprite(void)
{
    ...
    species = GetMonData(&party[gBattlerPartyIndexes[gActiveBattler]], MON_DATA_SPECIES);
...
static void PlayerPartnerHandleExpUpdate(void)
{
    ...
    else
    {
        ...
        LoadBattleBarGfx(1);
        GetMonData(&party[monId], MON_DATA_SPECIES);  // unused return value

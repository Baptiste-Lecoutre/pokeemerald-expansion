...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
    ...
    u8 monLevel, averageOpponentLevel = 0, playerLevel = GetMedianLevelOfPlayerParty();//GetHighestLevelInPlayerParty();
    s32 i, j;
    u8 monsCount;
        ...
        *(&gBattleStruct->absentBattlerFlags) = gAbsentBattlerFlags;
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            ...
            if (GetBattlerSide(i) == B_SIDE_OPPONENT && IsBattlerAlive(i))
                gBattleStruct->revealedEnemyMons |= 1u << gBattlerPartyIndexes[i];
        }

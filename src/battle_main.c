...
u8 IsRunningFromBattleImpossible(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
        return BATTLE_RUN_SUCCESS;
    if (GetBattlerAbility(gActiveBattler) == ABILITY_RUN_AWAY || BattlerHasInnate(gActiveBattler, ABILITY_RUN_AWAY))
        return BATTLE_RUN_SUCCESS;
...
u32 GetBattlerTotalSpeedStat(u8 battlerId)
{
    ...
    {
        if ((ability == ABILITY_SWIFT_SWIM || BattlerHasInnate(battlerId, ABILITY_SWIFT_SWIM)) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            speed *= 2;
        else if ((ability == ABILITY_CHLOROPHYLL || BattlerHasInnate(battlerId, ABILITY_CHLOROPHYLL)) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_SUN)
            speed *= 2;
        else if ((ability == ABILITY_SAND_RUSH   || BattlerHasInnate(battlerId, ABILITY_SAND_RUSH)) && gBattleWeather & B_WEATHER_SANDSTORM)
            speed *= 2;
        else if ((ability == ABILITY_SLUSH_RUSH  || BattlerHasInnate(battlerId, ABILITY_SLUSH_RUSH)) && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW)))
            speed *= 2;
    }
    ...
    if ((ability == ABILITY_QUICK_FEET  || BattlerHasInnate(battlerId, ABILITY_QUICK_FEET)) && gBattleMons[battlerId].status1 & STATUS1_ANY)
        speed = (speed * 150) / 100;
    else if ((ability == ABILITY_SURGE_SURFER || BattlerHasInnate(battlerId, ABILITY_SURGE_SURFER)) && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN)
        speed *= 2;
    else if ((ability == ABILITY_SLOW_START  || BattlerHasInnate(battlerId, ABILITY_SLOW_START)) && gDisableStructs[battlerId].slowStartTimer != 0)
        speed /= 2;
    else if ((ability == ABILITY_PROTOSYNTHESIS || BattlerHasInnate(battlerId, ABILITY_PROTOSYNTHESIS)) && gBattleWeather & B_WEATHER_SUN && highestStat == STAT_SPEED)
        speed = (speed * 150) / 100;
    else if ((ability == ABILITY_QUARK_DRIVE || BattlerHasInnate(battlerId, ABILITY_QUARK_DRIVE)) && gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN && highestStat == STAT_SPEED)
        speed = (speed * 150) / 100;
    ...
    // paralysis drop
    if (gBattleMons[battlerId].status1 & STATUS1_PARALYSIS && (ability != ABILITY_QUICK_FEET && BattlerHasInnate(battlerId, ABILITY_QUICK_FEET)))
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves)
{
    ...
    // Quick Draw
    if (!ignoreChosenMoves && ability1 == (ABILITY_QUICK_DRAW || BattlerHasInnate(battler1, ABILITY_QUICK_DRAW)) && !IS_MOVE_STATUS(gChosenMoveByBattler[battler1]) && Random() % 100 < 30)
    ...
    // Quick Draw
    if (!ignoreChosenMoves && (ability2 == ABILITY_QUICK_DRAW || BattlerHasInnate(battler2, ABILITY_QUICK_DRAW)) && !IS_MOVE_STATUS(gChosenMoveByBattler[battler2]) && Random() % 100 < 30)
    ...
    if (priority1 == priority2)
    {
        ...
        if (gProtectStructs[battler1].quickDraw && !gProtectStructs[battler2].quickDraw)
            ...
            strikesFirst = 0;
        else if ((ability1 == ABILITY_STALL || BattlerHasInnate(battler1, ABILITY_STALL)) 
                && (ability2 != ABILITY_STALL && !BattlerHasInnate(battler2, ABILITY_STALL)))
            strikesFirst = 1;
        else if ((ability2 == ABILITY_STALL || BattlerHasInnate(battler2, ABILITY_STALL)) 
                && (ability1 != ABILITY_STALL && !BattlerHasInnate(battler1, ABILITY_STALL)))
            strikesFirst = 0;
        else if ((ability1 == ABILITY_MYCELIUM_MIGHT || BattlerHasInnate(battler1, ABILITY_MYCELIUM_MIGHT)) 
                && (ability2 != ABILITY_MYCELIUM_MIGHT && !BattlerHasInnate(battler2, ABILITY_MYCELIUM_MIGHT)) && IS_MOVE_STATUS(gCurrentMove))
            strikesFirst = 1;
        else if ((ability2 == ABILITY_MYCELIUM_MIGHT || BattlerHasInnate(battler2, ABILITY_MYCELIUM_MIGHT)) 
                && (ability1 != ABILITY_MYCELIUM_MIGHT && !BattlerHasInnate(battler1, ABILITY_MYCELIUM_MIGHT)) && IS_MOVE_STATUS(gCurrentMove))
            strikesFirst = 0;
        else
        {
...
void SetTypeBeforeUsingMove(u16 move, u8 battlerAtk)
{
    ...
    else if (gBattleMoves[move].soundMove && (attackerAbility == ABILITY_LIQUID_VOICE || BattlerHasInnate(battlerAtk, ABILITY_LIQUID_VOICE)))
    {
        gBattleStruct->dynamicMoveType = TYPE_WATER | F_DYNAMIC_TYPE_2;
    }
